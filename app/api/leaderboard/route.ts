  ...
  try {
    ...
    const result = await sql(`
      SELECT
        ...
        u.name,
        u.pfp,
        COALESCE(SUM(s.points), 0) + (COALESCE(MAX(mc.count), 0) * 150) as total_score,
        ...
        GREATEST(
          ...
          COALESCE(MAX(mc.updated_at), '1970-01-01'::timestamp with time zone)
        ) as last_activity
      FROM users u
      LEFT JOIN scores s ON u.address = s.user_address
      LEFT JOIN mint_counts mc ON u.address = mc.user_address
      GROUP BY u.address, u.fid, u.name, u.pfp
