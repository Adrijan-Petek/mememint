...
export async function POST(request: NextRequest) {
  try {
    const { action, userAddress, points: rawPoints } = await request.json();
    ...
    let points: number = SCORING_CONFIG[action as ScoringAction];
    if (action === 'game' && typeof rawPoints === 'number' && Number.isFinite(rawPoints)) {
      const sanitized = Math.floor(Math.max(0, Math.min(rawPoints, 100000)));
      points = sanitized;
    }
