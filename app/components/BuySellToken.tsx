...
export default function SwapToken({ defaultMode }: { defaultMode?: 'buy' | 'sell' } = {}) {
  ...
  const shortTokenAddress = tokenAddress ? `${tokenAddress.slice(0, 6)}...${tokenAddress.slice(-4)}` : "—";
  ...
  return (
    <div className="mm-card mm-card-strong p-4 sm:p-6 shadow-xl">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div>
          <div className="text-sm text-white/60">Token Contract</div>
          <div className="flex items-center gap-2">
            <div className="text-base font-semibold text-white">{shortTokenAddress}</div>
            <button onClick={handleCopy} className="text-[11px] px-2 py-1 rounded-md border border-white/10 bg-white/5 text-white/70 hover:text-white">
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
          <div className="text-xs text-white/40">Base mainnet</div>
        </div>
        <div className="text-right">
          <div className="text-xs text-white/50 uppercase tracking-wide">Swap Mode</div>
          <div className="text-sm font-semibold text-white">{isEthInput ? `Buy ${tokenSymbol}` : `Sell ${tokenSymbol}`}</div>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        {!isConnected && <div className="text-xs text-white/40 mt-1">Connect your wallet to see balances and swap.</div>}
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="text-sm text-white/70 font-medium">Swap Direction</div>
        ...
        <div className="flex gap-3 flex-col sm:flex-row">
          <div className="flex-1">
            <label className="text-xs text-white/60 uppercase tracking-wide">Amount</label>
            <input value={fromAmount} onChange={e => setFromAmount(e.target.value)} inputMode="decimal" className="mt-2 w-full px-3 py-3 bg-gray-800/80 border border-blue-400/10 rounded-xl text-white placeholder-gray-400 text-base" placeholder="0.01" />
            <div className="mt-2 text-[11px] text-white/50">
              {isEthInput ? "You pay ETH and receive MEMEMINT." : `You sell ${tokenSymbol} for ETH.`}
            </div>
          </div>
          <div className="flex items-center gap-2 flex-col sm:flex-row">
            <button onClick={openSwapOnUniswap} className="whitespace-nowrap px-4 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-xl text-sm font-medium min-w-[150px]">Swap on Uniswap</button>
            <button onClick={handleFarcasterSwap} className="whitespace-nowrap px-4 py-3 bg-gray-700/80 text-white rounded-xl text-sm font-medium min-w-[150px]">Swap on Farcaster</button>
          </div>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 text-xs text-white/70 items-stretch">
          <div className="bg-white/5 rounded-xl p-3 border border-white/5 h-full flex flex-col">
            <div className="text-white font-medium mb-1">Uniswap</div>
            <div>Opens the official Uniswap swap in a new tab for full control.</div>
          </div>
          <div className="bg-white/5 rounded-xl p-3 border border-white/5 h-full flex flex-col">
            <div className="text-white font-medium mb-1">Farcaster</div>
            <div>Uses the native in‑app swap flow when available.</div>
          </div>
        </div>
      </div>
    </div>
