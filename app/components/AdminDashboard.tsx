...
export default function AdminDashboard({ isVisible, onClose }: AdminDashboardProps) {
  ...
  const handleSetGameFeeClick = async () => {
    if (!newGameFee) return;
    try {
      await handleSetGameFee();
    } catch (error) {
      console.error('Error setting game fee:', error);
    }
  };
        ...
        <div className="space-y-6">
          {activeTab === 'controls' && (
            <>
              ...
              {/* MemeBlast Fee Management */}
              <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-600">
                <h3 className="text-xl font-bold text-white mb-4">🎮 MemeBlast Fee</h3>
                <div className="text-gray-400 text-sm mb-3">
                  Current play fee: <span className="text-white font-semibold">{gamePlayFee || '0'} ETH</span>
                </div>
                <div className="flex gap-4">
                  <input
                    type="number"
                    step="0.000001"
                    placeholder="New play fee in ETH"
                    value={newGameFee}
                    onChange={(e) => setNewGameFee(e.target.value)}
                    className="flex-1 px-4 py-2 bg-slate-700 border border-slate-500 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-indigo-500"
                  />
                  <button 
                    onClick={handleSetGameFeeClick} 
                    disabled={!newGameFee || isLoading}
                    className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                  >
                    Set Game Fee
                  </button>
