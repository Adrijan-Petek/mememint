...
const MEMEBLAST_FEES_ADDRESS = CONTRACT_ADDRESSES.memeblastFees as `0x${string}`;
...
export function useAdminContract() {
  ...
  const { data: gamePlayFee } = useReadContract({
    address: MEMEBLAST_FEES_ADDRESS,
  ...
  return {
    ...
    gamePlayFee: gamePlayFee ? formatEther(gamePlayFee) : '0',
    dailyFreeLimit,
    treasury,
    totalMints,
    totalRevenue,
