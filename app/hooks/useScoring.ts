...
export function useScoring() {
  ...
  const addScore = useCallback(async (action: keyof typeof SCORING_CONFIG, userAddress: `0x${string}`, pointsOverride?: number) => {
    const points = typeof pointsOverride === 'number' ? pointsOverride : SCORING_CONFIG[action];
